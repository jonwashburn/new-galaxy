...
To demonstrate the concrete existence and reproducibility of our implementation, the code is available at \href{https://github.com/jonwashburn/gravity}{https://github.com/jonwashburn/gravity}. A top-level \texttt{production/} directory contains scripts, data link, and results. The workflow can be run with:
...
Path & Purpose \\
\midrule
production/scripts/build\_sparc\_master\_table.py & Build SPARC master table \\
production/scripts/ledger\_final\_combined.py & Core ILG pipeline (global-only) \\
production/scripts/reproduce\_048\_fit.py & Reproduce key median result \\
production/results/* & Outputs: logs, pkl, example figures \\
production/paper/* & Paper sources and compiled PDFs \\
\bottomrule
\end{tabular}
\end{table}
