...
\noindent\fbox{\parbox{0.97\linewidth}{\textbf{Significance.} \emph{Headline}\,: A global-only, falsifiable ILG/RS model shows that a single non-separable field kernel, derived from baryons and environment with no per-galaxy tuning, predicts galaxy rotation (and prospects to predict lensing), with reproducible 1D$\to$2D$\to$3D gains (3D $64^3$ median $\chi^2/N=2.876$). \newline \emph{Derived-not-fitted}\,: Every “extra element” ($R_d$, $\mathsf D$, $\ell_{\rm rec}$, $\xi$, $\zeta$, $g_{\rm ext}$) is an explicit functional of $\Sigma_b/E$; no $v_{\rm obs}$ leakage; identifiability supported by ablations and permutation/monotonic-trend checks; full repro bundle provided. \newline \emph{Unified kernel}\,: One SPD elliptic solve $K=(\mathcal I-\nabla\!\cdot(\ell_{\rm rec}^2\,\mathsf D\,\nabla))^{-\alpha}$ reduces exactly to the locked 1D law in axisymmetry and extends to 2D/3D, sketching a concrete path to rotation–lensing unification.}}
...
\medskip
\noindent\fbox{\parbox{0.97\linewidth}{\textbf{Limitations and Falsifiability (boxed).} Scope: The 1D axisymmetric weight is formally derived; 2D/3D field kernels and lensing are prospective and evaluated under a global-only empirical policy. Predictions are made with fixed global constants and no per-galaxy tuning.\newline\textbf{Null tests (expected failure signatures):}\newline\emph{(1) Rotation–lensing unification}\,: Stacked weak-lensing around SPARC-like disks shows no systematic convergence/shear enhancement at 20--50 kpc when the 3D kernel predicts one (or predicted sign flips); \emph{falsifies} the shared kernel link.\newline\emph{(2) Global-only ablations}\,: Removing anisotropy ($\mathsf D\!\to\!\mathsf I$), setting $g_{\rm ext}{=}0$, or enforcing constant-$\sigma$ does \emph{not} worsen medians in large samples; \emph{falsifies} the necessity of these RS-derived elements under the stated policy.\newline\emph{(3) Permutation/monotonic trends}\,: Shuffling feature labels leaves medians unchanged and preserves residual monotonicity vs $\xi$ and $r/R_d$; \emph{falsifies} identifiability of the feature layer.\newline Artifacts and precise thresholds are cataloged in \texttt{active/results/PROTOTYPE\_RESULTS.md}.}}
...
\medskip
\noindent\fbox{\parbox{0.97\linewidth}{\textbf{Derived, global-only policy (standard).} A model is admissible under our policy if it:\newline (i) expresses all “extra elements” as explicit functionals of $\{\Sigma_b,E\}$ with \emph{no} $v_{\rm obs}$ leakage; \newline (ii) uses one global configuration across the entire sample (no per-galaxy tuning); \newline (iii) shares masks and error models for fair baselines; \newline (iv) reports medians/means with 95\% CIs and publishes artifacts with checksums; \newline (v) passes ablation, permutation, and monotonic-trend checks; \newline (vi) publishes residual fields and, where applicable, uses the same kernel for rotation and lensing.}}
